...
export class TicketNewComponent implements OnInit {
  ...
  //ao finalizar  inclusao reseta o form
  @ViewChild("form")
  form: NgForm;

  ticket = new Ticket('', 0, 'title', 'status', 'priority', '', null, null, '', null);
  ...
  register(){
    ...
    this.ticketService.createOrUpdate(this.ticket).subscribe((responseApi: ResponseApi) => {
        this.ticket = new Ticket('', 0, 'title', 'status', 'priority', '', null, null, '', null);
        let ticketResponse: Ticket = responseApi.data;
        this.form.resetForm();
        this.showMessage({
          type: 'success',
  ...
  onFileChange(event): void{
    ...
    if(event.target.files[0].size > 2000000){
      this.showMessage({
        type: 'error',
        text: 'Maximun image size is 2MB'
      });
    }else{
